<cfcomponent>
	...
	<cffunction name="run">
		...
		<cfif (isDefined("app.config.routes.controller") and len(app.config.routes.controller))>
			<cfscript>
			try{
		...
		<cftry>
			...
			<cfcatch>
				<cfif cfcatch.errorCode eq "errorCode.badRequest">
					<cfset this.error(503,"The page could not be rendered","The object #cfcatch.missingfilename# could not be found")>
				</cfif>
