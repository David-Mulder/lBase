...
function loadStyleSheet(sheet, callback, reload, remaining) {
    //SHEET:href,title,type,contents,files,rootpath,entryPath,relativeUrls
    //writeFile("test","here:"+fGetKeys(sheet).toString());
    //var endOfPath = Math.max(name.lastIndexOf('/'), name.lastIndexOf('\\')),
    //    sheetName = name.slice(0, endOfPath + 1) + sheet.href,
    var sheetName = sheet.rootpath + "/" + sheet.href;
    var contents = sheet.contents || {},
        input = readFile(sheetName);
    writeFile("test",sheetName.replace(/[\w\.-]+$/, ''));
        
    contents[sheetName] = input;
        
    var parser = new less.Parser({
        paths: [sheetName.replace(/[\w\.-]+$/, '')],
        contents: contents
    });
...
//TODO: NEEDS TO BE EDITED TO TRANSLATE publicRootPath to rootPath to support imports!!!
function readFile(filename){
    var file = new java.io.File(filename);

    var fileContents = new java.lang.StringBuilder(file.length());
    var scanner = new java.util.Scanner(file);
    var lineSeparator = "\n";
    while(scanner.hasNextLine()) {        
        fileContents += scanner.nextLine() + lineSeparator;
    }
    scanner.close();
...
function parse(lesscss,rootpath,publicRootPath){
	var parser = new less.Parser({
	    rootpath: publicRootPath
	});
	var result;
	try {
		parser.parse(lesscss,function(e, root){
			if (e) {
				result = {haserror:true,e:e};
			}else{
				result = {haserror:false,css:root.toCSS({compress: false})};
			}
		});
	} catch(e) {
		result = {haserror:true,e:e};
